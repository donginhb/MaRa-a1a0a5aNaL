...
int
valid_cron_ntp(webs_t wp, char *value, struct variable *v)
{
    char tmp[TMP_LEN];
    char *val;
...
int
save_cron_ntp(webs_t wp, char *value, struct variable *v, struct service *services)
{
    char tmp[TMP_LEN], cron_ntp[TMP_LEN], cron_reboot[TMP_LEN];
    char tmp_ddns_countdown[TMP_LEN];
    char *ntp_enable, *ntp_interval;
    char *rule_set = "cron_rule", *ddns_countdown;
    int len, change = 0;
    int64_t map = 0;
    char reboot_interval[TMP_LEN], reboot_hour[TMP_LEN];
    ...
    if (!strcmp(value, "ntp")) {
        ...
        ezplib_get_attr_val("cron_rule", 0, "reboot_interval", reboot_interval, sizeof(reboot_interval), EZPLIB_USE_CLI);

        ezplib_get_attr_val("cron_rule", 0, "reboot_hour", reboot_hour, sizeof(reboot_hour), EZPLIB_USE_CLI);
...
int
valid_reboot(webs_t wp, char *value, struct variable *v)
{
    char tmp[TMP_LEN];
    char *val;
    if (!strcmp(value, "ntp")) {
...
int
save_reboot(webs_t wp, char *value, struct variable *v, struct service *services)
{
    char tmp[TMP_LEN], cron_ntp[TMP_LEN], cron_reboot[TMP_LEN];
    char tmp_ddns_countdown[TMP_LEN];
    char *reboot_interval, *reboot_hour;
    char *rule_set = "cron_rule", *ddns_countdown;
    int len, change = 0;
    int64_t map = 0;
    char ntp_enable[TMP_LEN];
    char ntp_interval[TMP_LEN];

    /* NTP */
    if (!strcmp(value, "ntp")) {
        /* NTP Enable */
        ezplib_get_attr_val("cron_rule", 0, "ntp_enable", ntp_enable, sizeof(ntp_enable), EZPLIB_USE_CLI);

        ezplib_get_attr_val("cron_rule", 0, "ntp_interval", ntp_interval, sizeof(ntp_interval), EZPLIB_USE_CLI);
