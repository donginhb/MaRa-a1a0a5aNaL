        ...
        int64_t *map)
{
    ...
    char *enable, *ssid, *bssid, *secmode, *channel;
#if defined(EZP_SUB_BRAND_APOLLO) || defined(EZP_PROD_BRAND_PROX)
    char *ip_type;
#endif
    ...
    idx = atoi(value);
    ssid = _get_apcli_web_variable(wp, idx, idx, "ssid");
    ...
    for (i = 0; i < num; i++) {
	    ...
	    if (strcmp(proto, old_proto)) {
                ...
                if (!strcmp(proto, "static")) {
                    ...
                    /* To set directip interface name by each dongle structure, not only use usb0  */
		    struct ezp_com_map_entry_type *map_ptr;
                    char *brand, *model;
		    int  nbrand, nmodel;
		    
		    /* Brand */
                    snprintf(tmp, sizeof(tmp), "wan%d_wwan_brand", i);
                    brand = websGetVar(wp, tmp, "");
                    nbrand = atoi(brand);

                    /* Model */
                    snprintf(tmp, sizeof(tmp), "wan%d_wwan_model", i);
                    model = websGetVar(wp, tmp, "");
                    nmodel = atoi(model);
                    
                    map_ptr=ezp_com_pointer_map[nbrand];
                    snprintf(buf2, sizeof(buf2), "%s", map_ptr[nmodel].IfName);		
		    nvram_set(buf1, buf2);
		...
		/* Changing from DHCP, the IP will reside in wan0_ipaddr until 
                ...
                if (!strcmp(proto, "directip")) {
                    /* To set directip interface name by each dongle structure, not only use usb0  */
                    struct ezp_com_map_entry_type *map_ptr;
                    char *brand, *model, *old_ifname;
                    int  nbrand, nmodel;
                    ...
                    /* get old interface name */
                    snprintf(tmp, sizeof(tmp), "wan%d_ifname", i);
                    old_ifname = nvram_safe_get(tmp);

                    map_ptr=ezp_com_pointer_map[nbrand];
                    snprintf(buf2, sizeof(buf2), "%s", map_ptr[nmodel].IfName);
                    if(strcmp(old_ifname, buf2)){
	                snprintf(buf1, TMP_LEN, "NUM=%d", i);
        	        config_preaction(&map, v, s, buf1, "");
                	/* Set ifname based on proto */
                	snprintf(buf1, sizeof(buf1), "wan%d_ifname", i);
