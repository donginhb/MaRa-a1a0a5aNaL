...
static struct variable wan_failover_variables[] = {
    {longname: "WAN Failback Enable", argv:ARGV("0", "1"), nullok: FALSE},
};
...
valid_lb(webs_t wp, char *value, struct variable *v)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if (*val == '1') {
            ...
            val = websGetVar(wp, tmp, "");
            if (valid_choice(wp, val,
                  &wan_failover_variables[WAN_FAILOVER_FAILBACK]) == FALSE) {
                return FALSE;
            }
#endif
        }
