...
<%prod_show_js("end", "wisp");%>
    ...
    function init() {
        ...
        for (var i = 0; i < <%nv_get("wan_num");%>; i++) {
            ...
            if (proto == 'wisp') {
                var wisp_ip_type = (EN("wan"+i+"_wisp_ip_type")[0].checked == true) ? 1 : 0; // 1:dchp, 0:static
                if (wisp_ip_type == '1') {
        ...
        <%prod_show_js_start("PROD_CAT_P|PROD_CAT_W", "1", "1" , "");%>
            alert('<%lang("The ethernet port will be configured as a WAN port.");%>');
        <%prod_show_js_end("PROD_CAT_P|PROD_CAT_W", "1", "1" , "");%>
        }
	  ...
	  <script type="text/javascript">do_page_path('<%lang("Setup");%>', '<%lang("WAN");%>')</script>
        <div id="contentsInfo">
          <form name="form" id="form" action="apply.cgi" method="post" >
            ...
            <% wan_show_setting(); %>
<%prod_show_html_start("PROD_CAT_P|PROD_CAT_W|PROD_CAT_M|PROD_CAT_N|PROD_CAT_U|PROD_CAT_I|PROD_CAT_A", "0", "2");%>
            <fieldset>
...
<%prod_show_html_end("PROD_CAT_P|PROD_CAT_W|PROD_CAT_M|PROD_CAT_N|PROD_CAT_U|PROD_CAT_I|PROD_CAT_A", "0", "2");%>
