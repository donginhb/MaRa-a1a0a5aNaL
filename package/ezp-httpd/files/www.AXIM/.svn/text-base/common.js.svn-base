...
function jquery_valid_number(I, M) {
    I = trim_unused_ws(I);
            ...
            for (i = 0; i < ret.length; i++) {
                ...
                if (pair[0] == "message") {
                    ...
                    if (val[0] == '1') {
                        ...
                        var command = "display_return(" + val[0] + ",'" + val[1] + "');";
...
function js_ajax_preaction(F, Vlan_columnNames, Vlan_rows, VlanPort_columnNames, VlanPort_rows, action, postaction) {
    var getstr = "";
	...
	// Vlan Data	
	for (var i=0; i<Vlan_rows.length ; i++ ){
		for (var j=0; j<Vlan_columnNames.length; j++ ){
			var str = "vlan_rule"+i+"_"+Vlan_columnNames[j].name + "=" + eval("Vlan_rows["+ i +"]."+Vlan_columnNames[j].name);
			getstr += str + "&";
		}
	}
	
	getstr += "vlan_rule_num=" + Vlan_rows.length +"&";

	// Vlan Port Data
	for (var i=0; i<VlanPort_rows.length ; i++ ){
		for (var j=0; j<VlanPort_columnNames.length; j++ ){
			var str = "vlanport_rule"+i+"_"+VlanPort_columnNames[j].name + "=" + eval("VlanPort_rows["+ i +"]."+VlanPort_columnNames[j].name);
			getstr += str + "&";
		}
	}
	getstr += "vlanport_rule_num=" + VlanPort_rows.length;
...
<%prod_show_js_start("PROD_CAT_C|PROD_CAT_G", "3", "7");%>
		if (pair[0] == 'bt_on') {
		if (pair[1] == '0') {
			E('bt_on').value = '0';
...
<%prod_show_js_end("PROD_CAT_C|PROD_CAT_G", "3", "7");%>
<%prod_show_js_start("PROD_CAT_C|PROD_CAT_T|PROD_CAT_D", "2", "2");%>
		if (pair[0] == 'bt_on') {
		if (pair[1] == '0') {
			E('bt_on').value = '0';
			E('bt_on').innerHTML = '<img src="picts/START.gif" width=25 height=25>';
...
<%prod_show_js_end("PROD_CAT_C|PROD_CAT_T|PROD_CAT_D", "2", "2");%>
