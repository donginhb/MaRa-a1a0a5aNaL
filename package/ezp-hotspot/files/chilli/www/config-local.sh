...
<input name="userurl" value="$FORM_userurl" type="hidden">
...
form() {
    echo "<form name=\"form2\" method=\"get\" action=\"$1\"><INPUT TYPE=\"hidden\" NAME=\"userurl\" VALUE=\"$FORM_userurl\">$2</form>"
}
form_start() {
    echo "<form name=\"form2\" method=\"get\" action=\"$1\"><INPUT TYPE=\"hidden\" NAME=\"userurl\" VALUE=\"$FORM_userurl\">$2"
}
form_end() {
    echo "</form>"
}
loginform() {
	...
	*)
	    ...
	    if [ "$1" = "yes" ]; then
	        form_start "login.chi" "$(uamfile login_form 1)"
            else
                form_start "login.chi" "$(uamfile login_form_hidden 1)"	        
	    fi

	    if [ "$2" =  "1" ]
            then
                uamfile "login_button_accept" 
            else
                uamfile "login_button" 
            fi
            form_end
	    ;;
    esac
...
domail() {
	...
	echo
	echo "-------------------------------------------------"
	echo "Powered by AXIM - http://www.aximcom.com/"
