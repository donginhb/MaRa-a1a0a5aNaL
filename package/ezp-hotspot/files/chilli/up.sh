...
then
	...
	# EZP: MASQUERADE
	eval $(ipcalc $HS_NETWORK $HS_NETMASK)
	ipt -I EZP_SNAT -t nat -s $HS_NETWORK/$PREFIX -j MASQUERADE
	# EZP: RT_CONN_MARK
	ipt -I EZP_RT_CONN_SET_MARK -t mangle -i $TUNTAP \
	                 -j MARK --set-mark 0xD0000000/0xF0000000
	#XXX: brctl hack
	brctl delif br0 ra1
    fi
fi
