...
static VOID ApCliCtrlCntFailPrint(
	IN UINT failureCnter,
	IN const CHAR* msgOut);
/*
    ==========================================================================
    Description:
	...
	OUT STATE_MACHINE_FUNC Trans[])
{
	...
	for (k = 0; k < MAX_APCLI_NUM; k++)
		pAd->ApCfg.ApCliTab[k].FailConntCounter = 0; 
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	USHORT ifIndex = (USHORT)(Elem->Priv);
	PULONG pCurrState = &pAd->ApCfg.ApCliTab[ifIndex].CtrlCurrState;
	UINT  *failureConntCnter = &pAd->ApCfg.ApCliTab[ifIndex].FailConntCounter; 
	...
	(*failureConntCnter)++;
	ApCliCtrlCntFailPrint(*failureConntCnter,"Use wrong security protocol or encryption method to connect to access-pointt");
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	(*failureConntCnter)++; 
	ApCliCtrlCntFailPrint(*failureConntCnter,"Connect to access-point with wrong password.");
	...
	IN const CHAR* msgOut)
{
	if(failureCnter == 1 )
	{
		DBGPRINT(RT_DEBUG_ERROR, ("WIFIConnect %s.\n",msgOut));
	}
