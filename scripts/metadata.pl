...
sub parse_target_metadata() {
	...
	while (<FILE>) {
		...
		/^Target-EZP-Platform:\s*(.+)\s*$/ and $target->{ezp_platform} = $1;
		/^Target-EZP-SWType:\s*(.+)\s*$/ and $target->{ezp_swtype} = $1;
...
EOF
	foreach my $target (@target) {
		$target->{subtarget} and print "\t\tdefault \"".$target->{ezp_platform}."\" if TARGET_".$target->{conf}."\n";
	}

    print <<EOF;
config TARGET_SW_TYPE
    string

EOF
	foreach my $target (@target) {
		$target->{subtarget} and print "\t\tdefault \"".$target->{ezp_swtype}."\" if TARGET_".$target->{conf}."\n";
	}
